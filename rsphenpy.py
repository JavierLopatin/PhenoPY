...
def Phenology(inData, dates=None, rollWindow=None, nGS=46,
              nan_replace=None, saveRaster=None):
    """
    Explanaition here....
    """
    ...
    phen = xr.DataArray(phen, dims=inData.dims, coords={
                        'time': xnew[:, 0, 0],
                        'y': inData.coords['y'],
                        'x': inData.coords['x']},
                        attrs=attrs)
    if saveRaster is None:
        ...
        import rasterio
        # save results
        with rasterio.open(saveRaster, "w", **inData.attrs) as dst:
            dst.write(phen)
